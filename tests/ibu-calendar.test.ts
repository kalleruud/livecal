...
describe("IBU Calendar Output", () => {
	test("generates valid ICS calendar with events and competitions", () => {
		...
		expect(ics).toContain(
			"SUMMARY:🏆 BMW IBU World Cup Biathlon - Kontiolahti",
		);
	...
	test("filters relay results to show only team results", () => {
		...
		// Individual leg result should NOT be shown as a separate result line
		// (but athletes are shown under their team)
		expect(ics).not.toContain("ROEISELAND M. (NOR)");
		expect(ics).not.toContain("ROEISELAND M. - 17:06.9");

		// Athletes should be shown under their team
		expect(ics).toContain("NORWAY - 1:07:06.2");
		expect(ics).toContain("ROEISELAND M.");
	});
